...
scale and the signal shape. It was verified that these systematic effects cancel
in the measurement of the ratio of the efficiencies $R_{\epsilon}$ and are
negligible with respect to the statistical error on the measurement. Only the
statistical error on the measurement is propagated in to the error on the
measurement of the ratio of the efficiencies $dR_{\epsilon}$.
Unlike in the previous chapter the statistical error on measurement of $R$ in
each pseudorapidity bin is propagated to the asymmetry as a systematic error. 
...
$R_{ij}$ and the incorrect charge rates $\omega_{i},\omega_{j}$ are related
by,
\begin{align}
R_{ij} &= 
\frac{
...
resolution of the electrons perfectly. Unfortunately the simulation does not take
in to account all the detector effects present in real data, so additional
residual corrections to the simulated electron \pT distributions are necessary. An
additional Gaussian smearing of the \pT distribution ($\sigma_{corr}$) and an electron
energy scale correction ($k_{corr}$) for each pseudorapidity bin is applied.

The values for $k_{i}$ and $\sigma_{i}$ are obtained from the
\HepProcess{\PZ\to\Pe\Pe} mass distribution. The \HepProcess{\PZ\to\Pe\Pe}
sample is again split in to 66 sub-samples for the combinations of pseudorapidity
...
the resolution function. The width and the mass of the Breit-Wigner function are
