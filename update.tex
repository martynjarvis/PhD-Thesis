\chapter[Electron Charge Asymmetry Update]{Update to the electron charge asymmetry
...
selections and it is vetoes if it contains a second lepton with
$\pT>\unit{15}{\GeV}$.
...
events that pass an anti-selection described in \TableRef{tab:antisel}.
...
bin are shown in \FigureRef{fig:data1}, \FigureRef{fig:data2}, \FigureRef{fig:data3} and
\FigureRef{fig:data4}.
The signal yield in each bin is summarised in \TableRef{tab:updatedsigyield} .
...
\bottomrule
  ...
  charge asymmetry  for an integrated luminosity of \unit{840}{\invpb}\cite{bendavid2011electron}. }
  \end{center}
\end{table}
...
given by the \EquationRef{eq:chargepdf}. The incorrect charge rates,
$\omega_i$, are extracted from a simultaneous fit to the 66 samples.
The resulting values of $\omega$ are shown in \TableRef{tab:mischarge}. The
...
\bottomrule
    \end{tabular}
    \caption{\label{tab:energyscalecorr}Bias in the charge asymmetry introduced by the electron energy scale and resolution.
...
real data for each $\eta$ bin\cite{bendavid2011electron}.}
...
\includegraphics*[width=0.80\textwidth]{updated_asym}
  \caption{\label{fig:updated_asym} Measured electron charge asymmetry with
predictions from CTEQ10, HERAPDF15, MSTW08NNLO and NNPDF2.2.}
  \end{center}
\end{figure}
...
HERAPDF, NNPDF and MSTW PDFs. Uncertainties on measured asymmetry are statistical and
