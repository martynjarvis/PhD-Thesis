...
convolution of the cross-section at the parton level, and the proton {PDF},
\begin{multline}
...
\subsubsection*{Proton l{PDF}} 
The proton {PDF} represents the number density of parton $a$ that has a momentum
...
The {PDF} for the valence quarks in the proton differ, the up-type quark
...
ratio of the up-type and the down-type {PDFs},
\begin{equation}
...
\FigureRef{wbos:pdfrat}. The up-type quark {PDF} dominates over the
...
At {LO} the \PWp is produced in the annihilation of a \Pup valance quark
...
However, due to the electroweak correlations the charged lepton from the decaying \PWp
...
electroweak correlations will shift the charge lepton rapidity distribution to higher
rapidities.

\FigureRef{fig:leptonrapidity} shows the {MC} generated rapidity distribution
...
The predictions are calculated using MCFM\cite{mcfm} {MC} tools interfaced with the
LHAPDF package\cite{lhapdf}. 
LHAPDF provides an interface to many different {PDF} sets. For the
following predictions, {PDF} from  the MSTW and CTEQ collaboration
are used.
...
using the `master equation'.\cite{Bourilkov:2006cj}.
...
predictions for a $\pT>\unit{25}{\GeV}$ with the {PDF} uncertainties for the
{PDF} sets MSTW08NLO\cite{martin2009parton} and CT10W\cite{lai2010vv}. The uncertainties are
$68\%$ confidence level. The predictions are generated using the {MCFM}
\cite{campbellmcfm} generator.
