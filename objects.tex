...
\begin{figure}[htbp]
  ...
  \caption{Distribution of the fraction of electron energy, $E^{e}$, radiated
away as bremsstrahlung photons, $\sum E_{brem}^{\gamma}$.}
\label{fig:brem}
\end{figure}
\todo{Arbitary Units spell out}
...
This is demonstrated in \FigureRef{fig:hybrid} \cite{meschi2001electron}.
...
known, so only the kinematic quantaties in the transverse place are usually
...
\label{sec:pf}
\todo{kinda unclear here, perhaps a diagram to explain it?}
