...
processes that are observed in nature, with the exception of the gravity and
...
\bottomrule
\end{tabular}
\caption{The fermions of the Standard Model. \label{tab:particles}}
\end{center}
\end{table}
...
a quark with a charge of $\nicefrac{2}{3}$ and the other with a charge of
$\nicefrac{-1}{3}$.
...
fermion field,
\begin{align}
\psi(x)       &\to \psi^{\prime}(x) = e^{i\omega} \psi(x),\\
...
The fundamental vertex for the neutral vertex is shown in
...
\begin{figure}[htbp]
  ...
  \caption{The fundamental vertex of neutral weak vertex.\label{fig:neutral}}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{The fundamental vertices for charged weak vertex.}
...
${T}^{a}$ are the three generators of $SU(2)$ group and $W^{a}_{\mu}$ are
...
contains the $SU(2)$ field strength where the index, $a$, runs from 1 to 3.
\begin{align*}
B^{\mu\nu}     &= \partial^{\mu} B^{\nu} - \partial^{\nu} B^{\mu},\\
W_{a}^{\mu\nu} &= \partial^{\mu} W_{a}^{\nu} - \partial^{\nu} W_{a}^{\mu} 
