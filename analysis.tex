...
select events.  As the luminosity increased even further, it was necesary to use
...
inverted selection is effective and will actually select events, it is necesary
to avoid using triggers that apply a selection on these variables.

The triggers used to select the events are sumarised in \TableRef{tab:triggers}
...
Electron candidates are only selected if the three methods unaminously agree on
its charge.
...
using only the \ac{GSF} track charge (red) and also requiring a unaminous
asignment of charge from all three methods (blue). 
...
if it contains a second lepton (an electron passing a loose selecton, or an
...
The uncertainty from \EquationRef{tab:statuncert} evaulated with \ac{MC} truth
...
from pseuodo-data experiments and the true MC value for the asymmetry divided by
...
will be a Gaussian centered at zero with a width of one.
The pull distribtions from 1000 pseudo-data experiments are shown in figure
...
The charge misassignment rates, $\omega^+$ and $\omega^-$ are the rate at which electrons are
misasigned a positive charge and identified as positrons and rate positrons are
identified as electrons respectivly.
The misassignment induces a dilution factor to the asymmetry as a
...
\end{equation}
If it is assumed that the misassignment rate of
...
The \TableRef{tab:tagprobe} shows the GSF tracking effiency and identifcation 
...
effect of the energy scale and signal shape is negligable
when compared to the statistical uncertainty of the measurement. Only the
statistical uncertainty is therfore propogated to the error in the ratio
$dR_\epsilon$,
\begin{equation}
  \label{eq:releff}
  \sigma_{\mathcal{A}} 
...
\end{equation}
The value of $R$ is measured in each pseudorapidity bin, as well as inclusivly
...
In order to reduce the systematic error due to the effect of the relaitve
efficiency the statistical error on the inclusive measurement of $R$ is
propogated to the asymmetry as a systematic error. The systematic error assigned
...
The rate of charge misassignment is obtained from \Zee samples selected from
...
It was found that the misassignment rate of electrons as poistrons is the same
as the rate of positrons as electrons.
The total Z yield from this sample is 6834 opposite sign \PZ candidates and 21
same sign \PZ candidates.
The sample is split in to 21 subsamples, representing
...
respectivly the probability of an event being same sign or oposite sign can be
written
...
%\]
\end{equation}
where $ \omega_{i},\omega_{j}$ are the charge misasignment rates and $
q_{i},q_{j}$ are the charges of the electrons.

The charge misassignment rates in each of the 6 pseudorapidity bins can then be
determined by a simultaneous fit to each of the 21 subsamples. The results are
shown in \FigureRef{tab:mischarge}. The charge asymmetry is corrected for these
values.
The statistical error on the measurement on $\omega$ is then propogated to the
...
To correct for this effect, a set of ad-hoc energy scale and resolution corrrections are
