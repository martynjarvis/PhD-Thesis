...
selection on the $\Delta\phi$ and $\Delta\eta$ variables.
...
where ``HLT\_Ele$X$'' indicates a selection requiring an electron with  $\Pt >
\unit{X}{\GeV}$.  ``Photon'' in the name indicates that the selection was
applied to ECAL superclusters rather than a reconstructed electron.  ``SW''
stands for small window, where window refers to the electron pixel-matching
window.  ``Cleaned'' indicates that spikes in the {ECAL} have been removed.  
...
as a compromise between ensuring suitable statistical accuracy while maintaining
...
events, which are mostly Drell-Yan (DY) events. 
...
shown in \FigureRef{fig:pfmet_dist}. There are two obvious peaks in the
...
$\PT > \unit{25}{\GeV}$ and $|\eta| < 2.4$\cite{baisini2010electron}.}
  \label{fig:pfmet}
  \end{center}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{The \ETm distribution on antiselected {MC} simulated events and selected {QCD} and \gjet events in each pseudorapidity bin\cite{baisini2010electron}.}
...
To validate the antiselection used, {QCD} monte carlo simulation (MC) samples are used. The
...
\label{sec:recoil}
The signal \ETm shape is obtained by modelling the recoil of the \PW boson. 
...
bin are shown the appendix in \FigureRef{fig:fit1} and \FigureRef{fig:fit2}.
...
for the barrel and endcap separatly and the full inclusive range\cite{baisini2010electron}.}
\label{tab:tagprobe}
\end{table}
...
The statistical error on the relative efficiency are treated as systematic
...
acceptance corrections. All unit in $\times 10^{-3}$\cite{baisini2010electron}.}
  \end{center}
\end{table}
...
generated using the signal and background \ETm shapes from MC samples
with the number of events that are expected in \unit{36.1}{\invpb} of
...
and the asymmetry measured with the chosen antiselection in MC pseudo data and real data for each eta bin\cite{baisini2010electron}.}
\label{tab:systQCD}
\end{center}
\end{table}
...
generated with the CTEQ 6.6 \cite{lai2010vv} uncertainty PDF set which contain
the central PDF and 44 error PDFs which contain the \unit{95}{\%} {CL} for each
of the 22 free parameters in the {PDF}. The maximum changes in the asymmetry
...
$2.0<|\eta|<2.4$ & 0.3\\
    \bottomrule
\end{tabular}
\caption{\label{tab:systelectroweak}Systematic uncertainty due to the electroweak \ETm shape used in the signal extraction method assigned to each eta bin\cite{baisini2010electron}.}
\end{center}
\end{table}
...
PDF models studied. At more central rapidites the CTEQ prediction is favoured
...
is compareable to the PDF uncertainty on the theoretical predictions.
